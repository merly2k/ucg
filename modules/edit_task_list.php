        ...
        $id=(int)$this->param[0];
        $dbz=new db;
        $zapros="select * FROM `urllist_sites` WHERE  `id`=$id LIMIT 1;";
        $dbz->get_rows($zapros);
        $tram=$dbz->result[0];

        $context="Изменение списочной задачи ";
        if(empty($_POST['ok'])){
        ...
        function form_edtask($tram) {
        extract($tram);
        //[base_url] [grab_mask][start_url] [end_url]  [pravilo][start]
        $ret="<form method='post' class='box'>
                <label>Раздел новостей<input type='text' placeholder='название раздела' name='razdel' id='razdel' value='$razdel' required />
                <label>адрес сайта<input type='url' placeholder='http://адрес сайта' name='base_url' id='base_url' value='$base_url' /></label>
                <label>что собирает<input type='search' placeholder='div.content>span.news' name='grab_mask' id='grab_mask' value='$grab_mask' /></label>
        ...
        function step2($tram) {
          ...
          //[base_url] [grab_mask][start_url] [end_url]  [pravilo][start]
          extract($_POST);
          $surl=first_uri($base_url, $grab_mask);
          //
            $ret="<form method='post' class='box'>
                $base_url<br>$surl
                ...
                <label>что собирает<input type='search' placeholder='div.content>span.news' value='$pravilo' name='pravilo' id='grab_mask' /></label>
                <input type='button' onclick='showSelectBlock();' name='select' value='выбрать'><br />
