        ...
        $context="добавление списочной задачи ";
        if(empty($_POST['ok'])){
        ...
        function form_addtask($param) {
          //[base_url] [grab_mask][start_url] [end_url]  [pravilo][start]
            $ret="<form method='post' class='box'>
                ...
                <label>адрес сайта<input type='url' placeholder='http://адрес сайта' name='base_url' id='base_url' /></label>
                <label>что собирает<input type='search' placeholder='div.content>span.news' name='grab_mask' id='grab_mask' /></label>
        ...
        function add_url_list_task($param) {
            ...
            extract($_POST);
            $db=new db();
            $zapros="INSERT INTO `urllist_sites` (`base_url`,`razdel`, `login`, `grab_mask`,`pravilo`) 
                        VALUES ('$base_urls','$razdel', '".$_SESSION['username']."', '$grab_mask','$pravilo');";
            $db->query($zapros);
